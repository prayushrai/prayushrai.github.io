"""Build a CSAB 2025 Special Round (final/Round 3) college predictor."""
import pandas as pd, json, os

CSV = '/sessions/amazing-cool-knuth/mnt/outputs/csab_2025_final_all.csv'
OUT = '/sessions/amazing-cool-knuth/mnt/outputs/csab_predictor.html'

# Institute -> State mapping (NITs + GFTIs that use Home/Other State quota)
INSTITUTE_STATE = {
    ...
    # GFTIs that use Home/Other State quota in CSAB
    ...
    "Delhi Technological University, Delhi": "Delhi",
...
<head>
<meta charset="utf-8">
<title>CSAB 2025 Special Round College Predictor (NIT / IIIT / GFTI)</title>
<style>
  * { box-sizing: border-box; }
  ...
  input:focus, select:focus { outline: none; border-color: #d97706; box-shadow: 0 0 0 3px rgba(217,119,6,0.15); }
  button { padding: 9px 18px; font-size: 14px; font-weight: 600; border: none; border-radius: 6px; cursor: pointer; background: #d97706; color: #fff; }
  button:hover { background: #b45309; }
  button.secondary { background: #eef2f7; color: #1a1a1a; }
  button.secondary:hover { background: #dee4ec; }
  .actions { display: flex; gap: 10px; flex-wrap: wrap; align-items: center; }
  .stats { background: #fef7e6; border-left: 4px solid #d97706; padding: 10px 14px; border-radius: 4px; font-size: 14px; margin-bottom: 14px; }
  ...
  .pill { display: inline-block; padding: 2px 8px; background: #fef7e6; color: #92400e; border-radius: 10px; font-size: 11px; font-weight: 600; margin-left: 4px; }
  .footer-note { font-size: 11px; color: #888; margin-top: 12px; }
  .filter-summary { font-size: 12px; color: #666; margin-top: 4px; }
  .badge { display:inline-block; padding:2px 8px; background:#fef7e6; color:#92400e; border-radius:4px; font-size:10px; font-weight:700; vertical-align:middle; margin-left:8px; }
</style>
</head>
<body>

<h1>CSAB 2025 Special Round College Predictor <span class="badge">CSAB SR-3</span></h1>
<div class="sub">Based on CSAB Special Round 3 (final) closing ranks for NITs, IIITs, and GFTIs &middot; Data scraped from admissions.nic.in/csabspl</div>

<div class="card">
  <div class="grid">
    <div><label>Student Name (optional)</label><input id="name" placeholder="e.g. Ankit Sharma"></div>
    <div><label>Rank (category rank if applicable)</label><input id="rank" type="number" placeholder="e.g. 50000" min="1"></div>
    ...
    <div><label>DASA quotas (for foreign-board candidates)</label>
      <select id="includeDasa">
        <option value="0">Exclude DASA (default)</option>
        <option value="1">Include DASA</option>
      </select>
    </div>
...
<div id="resultsSection" class="hidden">
  ...
  <div class="footer-note">
    Logic: The tool starts with your chosen bandwidth (default ±10%) around your rank and expands it in 10% steps until the number of options reaches the minimum you set (default 100). Quota rows (All India / Home State / Other State / J&K / Goa / DASA) are auto-selected based on your home state and DASA toggle.
  </div>
</div>
...
const ROWS = DATA.map(r => { const o = {}; COLS.forEach((c,i)=>o[c]=r[i]); return o; });
...
function isDasaQuota(q){ return q && q.indexOf('DASA') === 0; }

function computeEligible(seatType, gender, homeState, includeDasa) {
  ...
  return ROWS.filter(r => {
    ...
    const q = r.quota;
    if (q === 'All India') return true;
    if (q === 'Home State') return INST_STATE[r.inst] === homeState;
    if (q === 'Other State') return INST_STATE[r.inst] && INST_STATE[r.inst] !== homeState;
    if (q === 'Jammu & Kashmir (UT)') return homeState === 'Jammu & Kashmir';
    if (q === 'Home State for Goa') return homeState === 'Goa';
    if (isDasaQuota(q)) return !!includeDasa;
    return false;
  });
}
...
function run() {
  ...
  const includeDasa = document.getElementById('includeDasa').value === '1';

  const eligible = computeEligible(seatType, gender, homeState, includeDasa);
  ...
  document.getElementById('stats').innerHTML = `
    Student rank <b>${fmt(rank)}</b> · ${seatType} · ${gender==='F'?'Female':'Male'} · Home state: <b>${homeState}</b> · DASA: <b>${includeDasa?'Included':'Excluded'}</b><br>
    Eligible pool (before bandwidth): <b>${fmt(eligible.length)}</b> rows.<br>
...
function buildFilters() {
  ...
  fillMulti('progFilter', [...new Set(currentResults.map(r=>r.prog))].sort());
  fillMulti('quotaFilter', [...new Set(currentResults.map(r=>r.quota))].sort());
  document.getElementById('instSummary').textContent = `All ${[...new Set(currentResults.map(r=>r.inst))].length} institutes`;
  document.getElementById('progSummary').textContent = `All ${[...new Set(currentResults.map(r=>r.prog))].length} programs`;
}
...
function selectedMulti(id){ const s = document.getElementById(id); return new Set([...s.selectedOptions].map(o=>o.value)); }
function filteredRows() {
  ...
  return currentResults.filter(r =>
    types.has(r.type) && insts.has(r.inst) && progs.has(r.prog) && quotas.has(r.quota) &&
...
document.getElementById('typeChips').addEventListener('click', e=>{
  if (e.target.classList.contains('chip')) { e.target.classList.toggle('on'); renderTable(); }
});
...
function currentExportRows(){
  ...
  const dasa = document.getElementById('includeDasa').value==='1'?'Included':'Excluded';
  const header = ['Preference #','Institute Type','Institute','Academic Program','Quota','Seat Type','Gender','Opening Rank','Closing Rank'];
  const meta = [
    ['Student Name', name], ['Rank', rank], ['Category', seat], ['Gender', gen], ['Home State', hs],
    ['DASA quotas', dasa],
    ['Generated', new Date().toLocaleString('en-IN')],
    ['Source', 'CSAB 2025 Special Round 3 (final) closing ranks'],
    []
  ];
...
function downloadCSV(){
  ...
  triggerDownload(blob, `csab_preference_${sanitize(name)||'student'}_${rank||'rank'}.csv`);
}
function downloadXLSX(){
  const {meta, header, body, name, rank} = currentExportRows();
  let html = '<html><head><meta charset="utf-8"></head><body><table border="1">';
  meta.forEach(m => { html += '<tr>' + m.map(x=>`<td>${String(x||'')}</td>`).join('') + '</tr>'; });
  html += '<tr>' + header.map(h=>`<th style="background:#d97706;color:#fff">${h}</th>`).join('') + '</tr>';
  body.forEach(r => { html += '<tr>' + r.map((x,i)=>`<td${i>=7?' style="text-align:right"':''}>${x}</td>`).join('') + '</tr>'; });
  html += '</table></body></html>';
  const blob = new Blob(['\ufeff'+html], {type:'application/vnd.ms-excel'});
  triggerDownload(blob, `csab_preference_${sanitize(name)||'student'}_${rank||'rank'}.xls`);
}
function downloadPDF(){
  ...
  w.document.write(`
    <html><head><title>CSAB Preference list ${name||''}</title>
    <style>body{font-family:sans-serif;margin:20px;color:#222}h1{font-size:18px;margin:0 0 10px}.meta{font-size:12px;margin-bottom:14px;line-height:1.5}table{border-collapse:collapse;width:100%;font-size:11px}th,td{border:1px solid #888;padding:4px 6px}th{background:#d97706;color:#fff;text-align:left}</style></head>
    <body><h1>CSAB 2025 Special Round College Preference List</h1><div class="meta">${metaHtml}</div>
