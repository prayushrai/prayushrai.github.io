...
<link rel="icon" type="image/jpeg" href="dp.jpg">
<link rel="apple-touch-icon" href="dp.jpg">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=JetBrains+Mono:wght@500;700&display=swap" rel="stylesheet">
<script>
  // Apply persisted theme BEFORE first paint to avoid the light-mode flash.
  (function(){
    try {
      var t = localStorage.getItem('prayush-theme');
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero-tagline .heart { font-style: normal; }
  ...
  /* Scope the textbox-style reset away from checkboxes/radios so the
     browser keeps drawing the native tick / dot for those. */
  ...
  /* Standalone checkbox row (e.g. Management Quota) */
  .field-checkbox {
    display: flex; align-items: center; gap: 12px;
    padding: 12px 14px;
  ...
  .field-checkbox:hover { border-color: var(--line-strong); }
  .field-checkbox:has(input:checked) { border-color: var(--accent); background: var(--accent-soft); }
  ...
  .theme-toggle:hover { transform: rotate(15deg); box-shadow: var(--shadow-md); border-color: var(--line-strong); background: var(--surface); }
  .theme-toggle svg { width: 18px; height: 18px; }
  .theme-toggle .icon-sun { display: none; }
  [data-theme="dark"] .theme-toggle .icon-moon { display: none; }
  [data-theme="dark"] .theme-toggle .icon-sun  { display: block; }

  /* === Razorpay CTA — native styled link, both sticky + modal use it === */
  .rzp-cta {
    display: inline-flex;
    ...
    cursor: pointer;
    box-shadow: 0 8px 22px rgba(194, 65, 12, 0.32), 0 2px 6px rgba(194, 65, 12, 0.18);
    transition: transform 0.15s, box-shadow 0.18s, background 0.15s;
  }
  .rzp-cta:hover {
    background: var(--accent-hover);
    transform: translateY(-1px);
    box-shadow: 0 12px 28px rgba(194, 65, 12, 0.40), 0 3px 8px rgba(194, 65, 12, 0.22);
  }
  .rzp-cta:active { transform: translateY(0); }
  .rzp-cta-block { display: flex; width: 100%; }    /* full-width inside modal */

  /* === WhatsApp CTA — used inside the free-guidance modal === */
  .wa-cta {
    display: inline-flex;
    align-items: center; justify-content: center;
    gap: 10px;
    ...
    text-decoration: none; white-space: nowrap;
    cursor: pointer;
    box-shadow: 0 8px 22px rgba(37, 211, 102, 0.32), 0 2px 6px rgba(37, 211, 102, 0.18);
    transition: transform 0.15s, box-shadow 0.18s, background 0.15s;
  }
  .wa-cta:hover {
    background: #1ebe5a;
    transform: translateY(-1px);
    box-shadow: 0 12px 28px rgba(37, 211, 102, 0.40), 0 3px 8px rgba(37, 211, 102, 0.22);
  }
  ...
  /* Don't let content hide behind the floating CTA */
  body { padding-bottom: 90px; }

  /* === Counselling pop-up modal (fires 40s after Predict) === */
  .modal-overlay {
    ...
    padding: 20px;
    z-index: 200;
    animation: prayush-fade-in 0.18s ease-out;
  }
  .modal-overlay.hidden { display: none; }
  @keyframes prayush-fade-in { from { opacity: 0; } to { opacity: 1; } }
  .modal-content {
    background: var(--surface);
    border: 1px solid var(--line);
    border-radius: 16px;
    ...
    animation: prayush-pop-in 0.22s ease-out;
  }
  @keyframes prayush-pop-in { from { transform: scale(0.94); opacity: 0; } to { transform: scale(1); opacity: 1; } }
  .modal-close {
    position: absolute;
    top: 12px; right: 12px;
  ...
  .modal-close:hover { color: var(--ink-2); background: var(--surface-2); border: none; }
  .modal-title {
  ...
  [data-theme="dark"] {
    ...
    --ink-dim: #a1a1aa;
    --ink-faint: #71717a;
    --accent: #fb923c;            /* lighter orange for better contrast on dark */
    --accent-hover: #f97316;
    --accent-soft: rgba(251, 146, 60, 0.14);
    --accent-soft-2: rgba(251, 146, 60, 0.30);
    --success: #34d399;
  ...
  [data-theme="dark"] body { background: var(--bg); color: var(--ink-2); }
  ...
  [data-theme="dark"] .modal-content { box-shadow: 0 24px 60px rgba(0,0,0,0.6); }
  /* Pills / tags — recolour for dark backgrounds */
  [data-theme="dark"] .pill-NIT    { background: rgba(52,211,153,0.18); color: #6ee7b7; }
  [data-theme="dark"] .pill-IIIT   { background: rgba(168,85,247,0.20); color: #c4b5fd; }
  [data-theme="dark"] .pill-GFTI   { background: rgba(245,158,11,0.18); color: #fcd34d; }
  [data-theme="dark"] .pill-UPTAC  { background: rgba(217,119,6,0.20);  color: #fbbf24; }
  [data-theme="dark"] .pill-GGSIPU { background: rgba(132,204,22,0.18); color: #bef264; }
  [data-theme="dark"] .pill-JAC    { background: rgba(99,102,241,0.20); color: #a5b4fc; }
  [data-theme="dark"] .pill-JoSAA  { background: rgba(8,145,178,0.20);  color: #67e8f9; border-color: rgba(8,145,178,0.55); }
  [data-theme="dark"] .pill-CSAB   { background: rgba(219,39,119,0.20); color: #f9a8d4; border-color: rgba(219,39,119,0.55); }
  [data-theme="dark"] .pill-UPTAC-r  { background: rgba(217,119,6,0.20); color: #fcd34d; border-color: rgba(217,119,6,0.55); }
  [data-theme="dark"] .pill-GGSIPU-r { background: rgba(132,204,22,0.18); color: #bef264; border-color: rgba(132,204,22,0.55); }
  [data-theme="dark"] .pill-JAC-r    { background: rgba(99,102,241,0.20); color: #a5b4fc; border-color: rgba(99,102,241,0.55); }
  [data-theme="dark"] .chip[data-src="JoSAA"].on  { background: rgba(8,145,178,0.20); border-color: var(--josaa); color: #67e8f9; }
  [data-theme="dark"] .chip[data-src="CSAB"].on   { background: rgba(219,39,119,0.20); border-color: var(--csab); color: #f9a8d4; }
  [data-theme="dark"] .chip[data-src="UPTAC"].on  { background: rgba(217,119,6,0.20); border-color: var(--uptac); color: #fcd34d; }
  [data-theme="dark"] .chip[data-src="GGSIPU"].on { background: rgba(132,204,22,0.18); border-color: var(--ggsipu); color: #bef264; }
  [data-theme="dark"] .chip[data-src="JAC"].on    { background: rgba(99,102,241,0.20); border-color: var(--jac); color: #a5b4fc; }
  [data-theme="dark"] .tip { background: rgba(251,191,36,0.10); color: var(--warn-ink); border-color: rgba(251,191,36,0.30); }
  [data-theme="dark"] .badge { background: rgba(52,211,153,0.14); color: #6ee7b7; }
  [data-theme="dark"] .pill-quota { background: var(--surface-2); color: var(--ink-dim); }
  [data-theme="dark"] .pill-note  { background: rgba(168,85,247,0.20); color: #c4b5fd; }
  [data-theme="dark"] thead th { background: var(--surface-2); }
  [data-theme="dark"] .or-divider::before, [data-theme="dark"] .or-divider::after { background: var(--line); }
  [data-theme="dark"] tbody tr:hover td { background: var(--surface-2); }
  [data-theme="dark"] tbody td { border-bottom-color: var(--surface-3); }
  [data-theme="dark"] .vis-pill { background: var(--accent-soft); color: var(--accent); border-color: var(--accent-soft-2); }
  [data-theme="dark"] .field label .req { color: #fb7185; }
  [data-theme="dark"] .empty-hero { background: var(--surface-2); border-color: var(--line-strong); }
  ...
  @media (max-width: 640px) {
    ...
    .razorpay-sticky { bottom: 12px; }
    .modal-content { padding: 26px 22px 22px; }
...
<button id="themeToggle" class="theme-toggle" type="button" aria-label="Toggle dark/light theme" onclick="toggleTheme()">
  ...
  <section class="hero">
    <div class="hero-left">
      <h2 class="hero-headline">
        <span class="blue">FREE JEE Mains College Predictor</span>
        2026
      </h2>
      <p class="hero-subtitle">Built by <strong>Prayush Bhaiya</strong> to help students choose the best college at their rank.</p>

      <p class="hero-lead">The <b>first of its kind</b> tool in <b>INDIA</b> where you get college options from the <b>Top 5 College Counsellings</b> of India — including <b>JoSAA, CSAB, IPU, UPTAC &amp; JAC-Delhi</b>.</p>

      <p class="hero-tagline">Prayush bhaiya ka yahi hai <em>sapna</em> — har baccha <em>informed decision</em> lekar best deserved college tak pahuche, aur uske liye hi yeh college predictor banaaya hai <span class="heart">❤️</span></p>
    </div>
      ...
      <form class="form-card" onsubmit="event.preventDefault(); runPredict();">
        ...
        <div class="field">
          <label class="field-checkbox">
            <input type="checkbox" id="mgmtQuota">
            <span class="field-checkbox-text">Are you interested in Management Quota options?</span>
          </label>
        </div>
...
function runPredict(){
  ...
  // Reveal the counselling pop-up 30 s after the user predicts (once per session)
  scheduleCounsellingModal();
  // For high-budget OR low-rank+mid-budget leads, also schedule the free-WhatsApp
  // guidance modal at 60 s (replaces the counselling modal if still open).
  scheduleWhatsappModal(crl, budget);
}
...
function exportPayload(){
  ...
  const budget = $('budget').value || '—';
  const mgmtQuota = $('mgmtQuota').checked ? 'Yes' : 'No';
  const meta = [
    ...
    ['Management Quota Interest', mgmtQuota],
...
function toggleTheme(){
  ...
  try { localStorage.setItem('prayush-theme', next); } catch (_) {}
}

// === Counselling pop-up modal (30 seconds after Predict) ===
let counsellingModalShown = false;
let counsellingModalTimer = null;

function scheduleCounsellingModal(){
  if (counsellingModalShown) return;
  if (counsellingModalTimer) return;
  counsellingModalTimer = setTimeout(function(){
    counsellingModalTimer = null;
    if (counsellingModalShown) return;
    counsellingModalShown = true;
    var m = $('counsellingModal');
...
// === WhatsApp free-guidance modal (60s after Predict, conditional) ===
// Triggers when the lead is unlikely to convert to the ₹299 paid plan:
//   • Budget bracket upper-bound > 15 Lakhs (premium audience), OR
//   • CRL > 3 Lakhs AND budget upper-bound > 7 Lakhs (low rank, decent budget — likely needs management/private guidance).
let whatsappModalShown = false;
let whatsappModalTimer = null;

// Returns the upper-bound of the selected budget bracket, in lakhs.
//   "Under 5 Lakhs"      -> 5
//   "5-7 Lakhs"          -> 7
//   "7-10 Lakhs"         -> 10
//   "No budget constraints" -> 999
//   anything unparseable -> 0
function budgetUpperLakhs(budgetStr){
  if (!budgetStr) return 0;
  if (budgetStr === 'No budget constraints') return 999;
  if (budgetStr.indexOf('Under') === 0) {
    var u = budgetStr.match(/(\d+)/);
    return u ? parseInt(u[1], 10) : 0;
  }
  var m = budgetStr.match(/(\d+)\s*-\s*(\d+)/);
  return m ? parseInt(m[2], 10) : 0;
}

function shouldShowWhatsappModal(crl, budgetStr){
  var b = budgetUpperLakhs(budgetStr);
  if (b > 15) return true;
  if (crl > 300000 && b > 7) return true;
  return false;
}

function scheduleWhatsappModal(crl, budgetStr){
  if (whatsappModalShown) return;
  if (whatsappModalTimer) return;
  if (!shouldShowWhatsappModal(crl, budgetStr)) return;
  whatsappModalTimer = setTimeout(function(){
    whatsappModalTimer = null;
    if (whatsappModalShown) return;
    whatsappModalShown = true;
    // If counselling modal is still open from the 30s trigger, close it first
    // so the WhatsApp modal isn't visually stacked behind/over it.
...
document.addEventListener('keydown', function(e){
  if (e.key === 'Escape') { closeCounsellingModal(); closeWhatsappModal(); }
});
</script>

<!-- Sticky bottom CTA — links straight to the Razorpay-hosted payment page.
     Native <a> on purpose: the embed bundle renders inside an iframe and
     forces a "Powered by Razorpay" badge we can't style away from the parent. -->
...
<!-- Counselling pop-up modal (revealed 30s after Predict) -->
<div id="counsellingModal" class="modal-overlay hidden" onclick="if(event.target===this) closeCounsellingModal();">
  <div class="modal-content" role="dialog" aria-labelledby="counsellingModalTitle" aria-modal="true">
    <button class="modal-close" type="button" aria-label="Close" onclick="closeCounsellingModal()">×</button>
    <h3 class="modal-title" id="counsellingModalTitle">Get 1-1 Counselling by Prayush Bhaiya</h3>
    <p class="modal-subtitle">Personalised guidance, every step till the seat is yours.</p>
    <ul class="modal-list">
      <li>
        ...
        Helps in choosing best ROI College
      </li>
      <li>
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 8 7 12 13 4"/></svg>
        Helps in securing maximum Scholarship
      </li>
      <li>
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 8 7 12 13 4"/></svg>
        Unlimited Support till you get best college
      </li>
    </ul>
...
<!-- WhatsApp free-guidance modal (revealed 60s after Predict, conditional on budget/rank) -->
<div id="whatsappModal" class="modal-overlay hidden" onclick="if(event.target===this) closeWhatsappModal();">
  <div class="modal-content" role="dialog" aria-labelledby="whatsappModalTitle" aria-modal="true">
    <button class="modal-close" type="button" aria-label="Close" onclick="closeWhatsappModal()">×</button>
    <h3 class="modal-title" id="whatsappModalTitle">For free guidance, chat with us on WhatsApp.</h3>
    <p class="modal-subtitle">Talk to Prayush Bhaiya's team directly — no payment required.</p>
    <a class="wa-cta wa-cta-block" target="_blank" rel="noopener"
       href="https://wa.me/919667429207?text=Hi%20Prayush%20Bhaiya%2C%20I%20just%20used%20the%20College%20Predictor%20and%20would%20like%20free%20guidance.">
      <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        <path d="M19.05 4.91A9.82 9.82 0 0012.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38a9.9 9.9 0 004.74 1.21h.01c5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.91-7.01zM12.04 20.15h-.01a8.23 8.23 0 01-4.2-1.15l-.3-.18-3.12.82.83-3.04-.2-.31a8.22 8.22 0 01-1.26-4.38c0-4.54 3.7-8.24 8.25-8.24 2.2 0 4.27.86 5.83 2.42a8.18 8.18 0 012.42 5.83c0 4.55-3.7 8.23-8.24 8.23zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.12-.17.25-.64.81-.78.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.73-.65-1.23-1.46-1.37-1.71-.14-.25-.02-.38.11-.51.11-.11.25-.29.37-.43.12-.14.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.41-.42-.56-.42-.14-.01-.31-.01-.48-.01-.17 0-.43.06-.66.31-.23.25-.86.85-.86 2.07s.88 2.39 1 2.56c.12.17 1.74 2.65 4.21 3.72.59.25 1.05.4 1.41.51.59.19 1.13.16 1.55.1.47-.07 1.47-.6 1.67-1.18.21-.58.21-1.07.14-1.18-.06-.11-.22-.17-.47-.29z"/>
      </svg>
